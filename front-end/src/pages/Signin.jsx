...
export default function SignIn() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            // 1. ส่งคำขอเข้าสู่ระบบไปยัง Backend
            const data = await login(form);

            // 2. บันทึกข้อมูล Token และ User ลงใน LocalStorage
            localStorage.setItem('token', data.token);
            localStorage.setItem('user', JSON.stringify(data.user));

            console.log("Login Success, User Role:", data.user.role);
            
            // 3. ใช้ window.location.href แทน navigate เพื่อทำการ Hard Refresh หน้าจอ
            // วิธีนี้จะทำให้ Sidebar และ Navbar โหลดสิทธิ์ (Role) ใหม่ทันที
            window.location.href = "/dashboard";
            
        } catch (err) {
            // จัดการ Error กรณีข้อมูลผิดพลาด
