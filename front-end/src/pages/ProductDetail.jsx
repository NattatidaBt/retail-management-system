...
export default function ProductDetail() {
  ...
  const { id } = useParams(); // รับ ID สินค้าจาก URL parameter
  ...
  useEffect(() => {
    const fetchProductDetail = async () => {
      try {
        setLoading(true);
        // เรียก API ดึงข้อมูลสินค้าตาม ID
        const response = await api.get(`/products/${id}`);
        
        // จัดรูปแบบข้อมูลที่ได้จาก Database ให้เข้ากับ UI
        const data = response.data;
        setProduct({
          id: data.id,
          name: data.name,
          category: data.category_name || "ไม่ระบุ",
          weight: data.weight_g ? `${data.weight_g} กรัม` : "-",
          ...
          stock: data.stock,
          status: data.stock > 0 ? "ปกติ" : "สินค้าหมด",
